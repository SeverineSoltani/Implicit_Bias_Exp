function Main_Exp(timeline) {
	...
	if (demo === 1){
		...
		trainNumsRaw = [0,1,2];
		trainFaces = jsPsych.randomization.shuffle(['face_black1.jpeg', 'face_black2.jpeg', 'face_black3.jpeg', 'face_black4.jpeg', 'face_black5.jpeg']).slice(0,3);
		trainFaceNums = [1,2,3];
	...
	var bStimuliPage = {
		...
		stimulus: function(){ bStimuliIt++; getPosition();
				return "<img style='width:300px' src='IAT_Stimuli/" + stimuliB[bStimuliIt % stimuliB.length] + "'>" +
	...
	var bFixation = {
		type: 'html-keyboard-response',
		stimulus: function(){ return '<h1 style="font-size:2rem;">'+ 
	...
	var memStimuliPage = {
		type: "html-keyboard-response",
		stimulus: function(){
				return "<img style='width:300px' src='Individuation_Stimuli/" + trainFaces[facesUse[memShow % memMax]] + "'>" +
	...
	var countdownNextTrialDoneB = function(){ return {
		...
		on_finish: function(){
			...
			if (orderIt == 0)
			{
				jsPsych.addNodeToEndOfTimeline(generalInstructionScreen2(), jsPsych.resumeExperiment);	
			} else {
				jsPsych.addNodeToEndOfTimeline(bInstructionScreen(), jsPsych.resumeExperiment);
			}
